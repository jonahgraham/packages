...
cp ${WORKSPACE}/releng/org.eclipse.epp.config/tools/_mirrors.php _mirrors.php
...
# cd ${WORKSPACE}
# for i in $(find ${WORKSPACE}/archive -name '*.dmg-tonotarize')
# do
#    DMG_FILE=${i/-tonotarize/}
#    LOG=$(basename ${i}).log
#    echo "Starting ${DMG_FILE}" |& tee ${WORKSPACE}/${LOG}
#    ${WORKSPACE}/releng/org.eclipse.epp.config/tools/macosx-notarization-single.sh ${DMG_FILE} |& tee --append ${LOG} &
...
# if [[ -n `find ${WORKSPACE}/archive -name '*.dmg-tonotarize'` ]]; then
#    echo "Failed to notarize the following"
#    find ${WORKSPACE}/archive -name '*.dmg-tonotarize'
#    echo "The following files were found to not have been notarized" > tonotarize-list.log
#    find ${WORKSPACE}/archive -name '*.dmg-tonotarize' >> tonotarize-list.log
